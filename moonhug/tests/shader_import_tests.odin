...
test_shader_import_compiles_fixture :: proc(t: ^testing.T) {
	...
	header, spv, msl, props, parse_ok := engine._shader_artifact_parse(blob, context.temp_allocator)
...
test_shader_import_reflects_properties :: proc(t: ^testing.T) {
	...
	header, _, _, props, parse_ok := engine._shader_artifact_parse(blob, context.temp_allocator)
	testing.expect(t, parse_ok, "artifact failed to parse")
...
test_shader_artifact_parse_roundtrip :: proc(t: ^testing.T) {
	...
	parsed, p_spv, p_msl, props, ok := engine._shader_artifact_parse(blob[:], context.temp_allocator)
	...
	_, _, _, _, bad := engine._shader_artifact_parse(blob[:len(blob) - 1], context.temp_allocator)
