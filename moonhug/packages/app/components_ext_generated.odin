...
register_app_components :: proc() {
	sync.once_do(&_register_app_components_once, proc() {
		...
		engine.component_register(engine.Component_Desc{
			type_key  = .Lifetime,
			type_guid = engine.Lifetime__Guid,
			tid       = typeid_of(Lifetime),
			ptr_tid   = typeid_of(^Lifetime),
			pool_create = proc() -> rawptr { p := new(engine.Pool(Lifetime)); engine.pool_init(p); return p },
		...
		engine.component_register(engine.Component_Desc{
			type_key  = .Player,
			type_guid = engine.Player__Guid,
			tid       = typeid_of(Player),
			ptr_tid   = typeid_of(^Player),
			pool_create = proc() -> rawptr { p := new(engine.Pool(Player, 10)); engine.pool_init(p); return p },
			...
			on_destroy = proc(ptr: rawptr) { on_destroy_Player(cast(^Player)ptr) },
		})
		engine.component_register(engine.Component_Desc{
			type_key  = .Projectile,
...
lifetimes :: proc(w: ^engine.World) -> ^engine.Pool(Lifetime) {
	return cast(^engine.Pool(Lifetime)) w.ext_pools[engine.TypeKey.Lifetime]
}

players :: proc(w: ^engine.World) -> ^engine.Pool(Player, 10) {
	return cast(^engine.Pool(Player, 10)) w.ext_pools[engine.TypeKey.Player]
}
...
get_comp :: proc(tH: engine.Transform_Handle, $T: typeid) -> (engine.Owned, ^T) {
	when T == DemoMenu {
		...
		t := engine.pool_get(&w.transforms, engine.Handle(tH))
		if t == nil do return {}, nil
		owned, _ := engine.transform_find_comp(t, .Lifetime)
		if owned.handle.type_key == engine.INVALID_TYPE_KEY do return owned, nil
		pool := lifetimes(w)
		if pool == nil do return owned, nil
		return owned, engine.pool_get(pool, owned.handle)
		...
		t := engine.pool_get(&w.transforms, engine.Handle(tH))
		if t == nil do return {}, nil
		owned, _ := engine.transform_find_comp(t, .Player)
		if owned.handle.type_key == engine.INVALID_TYPE_KEY do return owned, nil
		pool := players(w)
		if pool == nil do return owned, nil
		return owned, engine.pool_get(pool, owned.handle)
