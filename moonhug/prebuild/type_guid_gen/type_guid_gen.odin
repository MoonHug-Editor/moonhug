...
_generate_type_key :: proc(entries: []_TypeGuidRow, w: ^db.World) -> bool {
	// TypeKey is core vocabulary (moonhug:engine/core): packages index by it
	// without importing the engine. The __Guid globals stay in package engine —
	// Odin aliases types and procs but not variables, and both packages and
	// generated registrations reference them as engine.<T>__Guid.
	{
		...
		db.emit(w, "moonhug/engine/core/type_key_generated.odin", strings.to_string(b))
	}
	{
		b := strings.builder_make()
		defer strings.builder_destroy(&b)
