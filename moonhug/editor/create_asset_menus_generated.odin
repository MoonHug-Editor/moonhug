...
__create_asset__AnimationClip :: proc() {
	full_path, _ := filepath.join({projectViewData.currentPath, "New Animation.anim"}, context.temp_allocator)
	instance := engine.create_instance_by_type_key(engine.TypeKey.AnimationClip)
	serialization.write_asset_to_path(full_path, engine.get_guid_by_type_key(engine.TypeKey.AnimationClip), instance)
}
...
register_create_asset_menus :: proc() {
	menu.add_menu_item("Assets/Create/Animation", "", __create_asset__AnimationClip, -5)
