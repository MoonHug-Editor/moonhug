...
_create_shader :: proc(stage: sdl.GPUShaderStage, num_samplers, num_uniform_buffers: u32) -> ^sdl.GPUShader {
	...
	if .MSL in formats {
		...
		blob := stage == .VERTEX ? _WORLD_VERT_MSL : _WORLD_FRAG_MSL
		info.code = raw_data(blob)
		info.code_size = len(blob)
	} else if .SPIRV in formats {
		info.format = {.SPIRV}
		info.entrypoint = "main"
		blob := stage == .VERTEX ? _WORLD_VERT_SPV : _WORLD_FRAG_SPV
		info.code = raw_data(blob)
		info.code_size = len(blob)
	} else {
		return nil
